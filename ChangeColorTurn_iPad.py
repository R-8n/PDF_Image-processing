...
if check_password():
    ...
    st.markdown("""
        ...
        /* ▼▼ 行間を極限まで詰める魔法 ▼▼ */
        /* Streamlitが勝手に入れるブロック間の隙間をゼロにする */
        [data-testid="stVerticalBlock"] { gap: 0rem !important; }
        [data-testid="column"] { gap: 0rem !important; }
        
        /* 各パーツの外側の余白をマイナスにしてめり込ませる（隙間をなくす） */
        [data-testid="stElementContainer"] { margin-bottom: -12px !important; }
        
        /* スライダー特有の上下の無駄なスペースを極限まで削る */
        .stSlider { padding-top: 0px !important; padding-bottom: 0px !important; margin-top: -15px !important; margin-bottom: -15px !important; }
        
        /* 数値入力ボックスの高さと余白調整 */
        .stNumberInput { padding-bottom: 0px !important; margin-bottom: -5px !important; }
        
        /* 区切り線（divider）の上下余白を縮小 */
        hr { margin-top: 15px !important; margin-bottom: 15px !important; border-color: #555 !important; }
        
        /* 保存ボタン（巨大なオレンジボタン） */
        button[kind="primary"] { background: linear-gradient(180deg, #FFB347 0%, #FF9500 100%) !important; color: #1A1A1A !important; min-height: 60px !important; font-size: 22px !important; font-weight: bold; border-radius: 25px; margin-top: 15px !important; margin-bottom: 10px !important; }
        
        /* 小さいボタン（◀▶▲▼ や 角度ボタン） */
        .stButton > button { background-color: #444 !important; color: white !important; height: 100%; min-height: 35px !important; padding: 0px 5px !important; }
        
        /* プレビュー画像枠 */
        [data-testid="stImage"] { border: 2px solid #FF9500; border-radius: 15px; padding: 10px; margin-top: 10px; }
        </style>
    ...
    st.markdown("""
        <div style="text-align: center; margin-top: 10px; margin-bottom: 10px;">
            <span style="font-family: 'Arial Black', sans-serif; font-size: 36px; color: #FF9500; font-weight: bold;">
                PDF ページ並び替えツール
            </span>
        </div>
    """, unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        ...
        with col1:
            ...
            bc1.slider("明度", 0.0, 100.0, 50.0, key="b_sl", step=1.0, on_change=sync_b_sl, label_visibility="collapsed")
            bc2.number_input("明度数", 0.0, 100.0, 50.0, key="b_nm", on_change=sync_b_nm, label_visibility="collapsed", step=1.0)

            st.write("コントラスト")
            cc1, cc2 = st.columns([3, 1])
            cc1.slider("コントラスト", 0.0, 100.0, 50.0, key="c_sl", step=1.0, on_change=sync_c_sl, label_visibility="collapsed")
            cc2.number_input("コン数", 0.0, 100.0, 50.0, key="c_nm", on_change=sync_c_nm, label_visibility="collapsed", step=1.0)

            st.write("彩度")
            sc1, sc2 = st.columns([3, 1])
            sc1.slider("彩度", 0.0, 100.0, 50.0, key="s_sl", step=1.0, on_change=sync_s_sl, label_visibility="collapsed")
            sc2.number_input("彩度数", 0.0, 100.0, 50.0, key="s_nm", on_change=sync_s_nm, label_visibility="collapsed", step=1.0)
            ...
            rc1.slider("回転", 0.0, 359.9, 0.0, key="rot_sl", step=0.1, on_change=sync_rot_sl, label_visibility="collapsed")
            rc2.button("▼", on_click=rot_down, key="btn_rot_d")
            rc3.number_input("回転数", -1.0, 360.0, 0.0, key="rot_nm", on_change=sync_rot_nm, label_visibility="collapsed", step=0.1)
