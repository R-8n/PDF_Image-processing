...
if check_password():
    ...
    if uploaded_file is not None:
        ...
        with col1:
            ...
            bc1.slider("明度", 0.0, 100.0, 50.0, key="b_sl", step=0.1, on_change=sync_b_sl, label_visibility="collapsed")
            bc2.button("▼", on_click=b_down, key="btn_b_d")
            bc3.number_input("明度数", 0.0, 100.0, 50.0, key="b_nm", on_change=sync_b_nm, label_visibility="collapsed", step=0.1)
            bc4.button("▲", on_click=b_up, key="btn_b_u")

            # ★コントラスト (0.1ステップ ＆ 上下ボタン連動)
            st.write("コントラスト")
            cc1, cc2, cc3, cc4 = st.columns([3.5, 1, 2.5, 1])
            cc1.slider("コントラスト", 0.0, 100.0, 50.0, key="c_sl", step=0.1, on_change=sync_c_sl, label_visibility="collapsed")
            cc2.button("▼", on_click=c_down, key="btn_c_d")
            cc3.number_input("コン数", 0.0, 100.0, 50.0, key="c_nm", on_change=sync_c_nm, label_visibility="collapsed", step=0.1)
            cc4.button("▲", on_click=c_up, key="btn_c_u")

            # ★彩度 (0.1ステップ ＆ 上下ボタン連動)
            st.write("彩度")
            sc1, sc2, sc3, sc4 = st.columns([3.5, 1, 2.5, 1])
            sc1.slider("彩度", 0.0, 100.0, 50.0, key="s_sl", step=0.1, on_change=sync_s_sl, label_visibility="collapsed")
            sc2.button("▼", on_click=s_down, key="btn_s_d")
            sc3.number_input("彩度数", 0.0, 100.0, 50.0, key="s_nm", on_change=sync_s_nm, label_visibility="collapsed", step=0.1)
